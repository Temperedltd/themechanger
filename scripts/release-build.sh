...
copy_optional_directory() {
	...
	if [[ -d "$source_dir/$directory" ]]; then
		cp -R "$source_dir/$directory" "$package_dir/$directory"
	fi
}
...
build_zip() {
	...
	local output_dir="$2"
	local version="$3"
	local package_name="tempered-themechanger"
	...
	copy_required_file "$source_dir" "$package_dir" 'tempered-themechanger.php'
	...
	copy_required_file "$source_dir" "$package_dir" 'uninstall.php'
	...
	rm -f "$package_dir/includes/runtime-spike.php"
	copy_optional_directory "$source_dir" "$package_dir" 'assets'
	copy_optional_directory "$source_dir" "$package_dir" 'languages'
